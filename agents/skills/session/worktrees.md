...
<traps>
PR commands must run from the main repo directory, not the worktree, because `gh` and `glab` misdetect the repo context inside worktrees. Use `--head <branch>` to target the worktree branch.
